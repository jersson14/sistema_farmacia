...
function addSociete( $nom, $adresse, $logo, $ext_logo  )
{
    $x1 = 30;
    $y1 = 4;
    // Inserta el logo validando el tipo para evitar errores en PHP 8
    if (!empty($logo) && file_exists($logo)) {
        $tipoLogo = '';
        if (!empty($ext_logo)) {
            $tipoLogo = strtolower(trim($ext_logo));
            if ($tipoLogo === 'jpeg') {
                $tipoLogo = 'jpg';
            }
        }
        ...
        if ($tipoLogo !== '') {
            $this->Image($logo, 5, 3, 25, 0, $tipoLogo);
        } else {
...
function addTVAs( $impuesto, $total_venta, $simbolo )
{
    ...
    $factor = 1 + ((float)$impuesto / 100);
    if ($factor <= 0) {
        $factor = 1;
    }
    $total=(float)$total_venta/$factor;
